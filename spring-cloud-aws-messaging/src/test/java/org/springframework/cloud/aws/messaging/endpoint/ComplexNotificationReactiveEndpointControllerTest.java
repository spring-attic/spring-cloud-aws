...
import org.springframework.cloud.aws.messaging.endpoint.ComplexNotificationReactiveEndpointControllerTest.Config;
...
class ComplexNotificationReactiveEndpointControllerTest {
	...
	void subscribe_subscriptionConfirmationRequestReceived_subscriptionConfirmedThroughSubscriptionStatus()
		...
		// Act
		this.webTestClient.post().uri("/myComplexTopic")
				.header("x-amz-sns-message-type", "SubscriptionConfirmation")
				.bodyValue(subscriptionRequestJsonContent).exchange().expectStatus()
				.isNoContent();
	...
	void notification_notificationReceivedAsMessageWithComplexContent_notificationSubjectAndMessagePassedToAnnotatedControllerMethod()
		...
		this.webTestClient.post().uri("/myComplexTopic")
				...
				.bodyValue(notificationJsonContent).exchange().expectStatus()
				.isNoContent();
