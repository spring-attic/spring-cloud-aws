 ...
 */
public class NotificationMessageReactiveHandlerMethodArgumentResolver
		extends AbstractNotificationMessageReactiveHandlerMethodArgumentResolver {

	private final List<HttpMessageConverter<?>> messageConverters;

	public NotificationMessageReactiveHandlerMethodArgumentResolver() {
	...
	protected Object doResolveArgumentFromNotificationMessage(JsonNode body,
		...
		MediaType mediaType = getMediaType(body);
		String messageContent = body.findPath("Message").asText();

		for (HttpMessageConverter<?> converter : messageConverters) {
			if (converter.canRead(methodParameter.getParameterType(), mediaType)) {
				try {
					final ByteArrayHttpInputMessage inputMessage = new ByteArrayHttpInputMessage(
							messageContent, mediaType, request);

					return converter.read((Class) methodParameter.getParameterType(),
							inputMessage);
				}
				catch (Exception e) {
