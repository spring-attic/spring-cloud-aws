...
// general
// - add numbers to chapters (structure based on title font size is difficult to track)
// - add table of contents (gives immediate overview)
// - add reference for all xml elements and attributes (appendix)
// - use consistent naming (case) for terms like "Maven", "Java", "Amazon Web Services", "CloudFormation"
...
// - is Spring Cloud AWS Messaging only directly dependent on Spring Cloud AWS Context, while Spring Cloud AWS JDBC is
//   only directly dependent on Spring Cloud AWS Core as image implies?
// - is Spring Cloud Context based on Spring Cloud AWS Core or are they really independent of each other?
...
// - why do xml elements repeat "context" in element name? could just be "aws-context:region name=''" instead
...
// - use access key and secret key (without dash) if used in paragraphs (not directly referring to xml attribute),
//   otherwise if used as reference to attribute name, use monospaced font
...
// - explicitly mention that this approach is only supported if application is running on ec2 instance
...
// - is fallback configuration based on order of xml elements, or is instance-based approach always given preference?
...
// - is global configuration always needed, even if all services use specific region configuration?
// - is there also support for fallback region settings (e.g. use automatic region configuration, if available,
     otherwise fallback to explicit configuration), as for credentials configuration
...
// - recommendation not to mix regions: explain why
...
// - where is special environment variable coming from? from spring cloud aws? from spring environment abstraction?
...
// - SimpleConfigurationBean gives impression of being part of Spring Cloud AWS, but is only example bean, so maybe
//   better use different package name (and shorter one)
...
// - paths to nested instance metadata properties: when to use "/", when "."?
...
// - is format if user data defined by Amazon or Spring Cloud AWS?
// - how to escape special characters ":" and ";"?
// - precedence in case of name conflicts (e.g. user data with same name as instance data)?
...
// - how will changes to instance tags be visible to application? upon next read? how? or via callback?
...
// - attributes for ec2 clients: use "-client" suffix in attribute name
...
// - should custom client not be nested bean of stack-configuration element? or referred to via id?
//   "amazon-cloud-formation" attribute has empty value ...
...
// - what if logical queue name is used in send api, but no resource id resolver is configured?
...
// - why not configure a queue messaging template bean with default name in case no one is defined explicitly? or is
//   this part of spring boot support?
...
// - representation of domain object in message is nevertheless relevant, as it needs to be understood by consumer, so
//   "not concerned about details of how message is represented" is a bit misleading ...
...
// - paragraph is a duplication (with more details) of paragraph under message converters: merge issue?
...
// - explain in introduction that queue listener container is using thread pool in order to understand why task executor
//   can be configured
...
// - why element "annotation-driven-queue-listener" has attribute with same name? should it rather read
//   "queue-messaging-template"?
// - what if no queue messaging template bean is defined? is one created automatically? exception?
...
// - different order of message and subject in example code snippet
// - NotificationMessagingTemplate.sendNotification() should use subject as first parameter, and message as second
//   parameter (not vice-versa)
...
// - does spring cloud aws _only_ support http/https (unlike aws with sqs, email, ...)?
...
// - why is caching not in a separate module (instead of having multiple namespaces in same module)?
...
// - why element "cache-ref" is used, and not element "cache" with attribute "ref"?
// - do all caches have to be of type memcached?
...
// - is name of @Cacheable annotation optional, if only one cache is defined?
...
// - what if other memcached cache is used with different driver (not one from aws) in addition in a mixed-caching
//   scenario? does spring cloud aws put any constraints on that scenario?
...
// - why is element for jdbc namespace is called "jdbc" and not "aws-jdbc"
// - why is element for data source written in camel case ("dataSource")
...
// - code example for SimpleDatabaseService: interface not really relevant for example ...
...
// - is "pool-attributes" element provided by spring cloud aws jdbc module? or by spring already?
// - why is pool element named with "-", but attributes in camel case?
...
// - behaviour if multiple read replicas are configured? round-robin?
...
// - example repeats "aws:cloudformation" in db tag name: correct?
...
// - why is mail support not in separate module instead of having multiple xml namespaces in context module?
...
// - should be Resource instead of ResourceLoader as based interface for WritableResource?
