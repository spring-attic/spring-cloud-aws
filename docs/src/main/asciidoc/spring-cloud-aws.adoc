...
1. `STSAssumeRoleSessionCredentialsProvider` if `cloud.aws.credentials.sts.role-arn` and `cloud.aws.credentials.sts.role-session-name` are provided
2. `AWSStaticCredentialsProvider` if `cloud.aws.credentials.access-key` is provided
3. `EC2ContainerCredentialsProviderWrapper` if `cloud.aws.credentials.instance-profile` is set to `true`
4. `ProfileCredentialsProvider` if `cloud.aws.credentials.profile-name` is provided
...
|arn:aws:iam::01234567890:role/sts-test
|The arn of the role to be assumed

|cloud.aws.credentials.sts.role-session-name
|testRoleSessionName
|An identifier for the assumed role session

|cloud.aws.credentials.sts.role-session-duration-seconds
|900
